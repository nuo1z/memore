...
- **评论双向同步**：新增评论（Comment）的完整双向同步功能。本地创建/修改/删除的评论自动同步到远端，远端的评论变更也会拉取到本地。评论同步在每次 Push+Pull 完成后自动执行，无需额外开关。

- **侧栏标签默认开启树模式**：新安装默认以树形结构展示标签层级（如 `parent/child`），无需手动切换。

### 修复与优化

- **修复删除笔记后不触发同步**：删除本地笔记后，同步系统不会自动将删除操作推送到远端。现已在 `useDeleteMemo` 成功回调中添加同步触发。

- **修复评论块宽度过窄**：MemoDetail 页面中评论块未设置 `w-full`，导致安卓等窄屏设备上评论显示过窄。

- **同步性能优化**：
  - 默认 pageSize 从 100 增大到 200，减少分页请求次数
  - 附件同步改为并行处理（最多 3 个并发），显著加快含多附件笔记的同步速度
  - maxPages 默认值从 10 增大到 20，支持更大规模的笔记库

- **Android 图标优化**：前景图标占比从 65% 缩小到 50%，图标更美观居中。
