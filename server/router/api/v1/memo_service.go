...
func (s *APIV1Service) ListMemos(ctx context.Context, request *v1pb.ListMemosRequest) (*v1pb.ListMemosResponse, error) {
	...
	} else if request.State == v1pb.State_NORMAL {
		state := store.Normal
		memoFind.RowStatus = &state
	}
	// STATE_UNSPECIFIED (0): no RowStatus filter → returns all states
...
func (s *APIV1Service) UpdateMemo(ctx context.Context, request *v1pb.UpdateMemoRequest) (*v1pb.Memo, error) {
	...
	// Auto-bump updatedTs for local changes (e.g., pinning, archiving) that
	// don't explicitly include "update_time" in the mask. This ensures the
	// sync engine can detect the change via timestamp comparison.
