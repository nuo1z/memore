import { CogIcon, DatabaseIcon, LibraryIcon, LucideIcon, Settings2Icon } from "lucide-react";
...
type SettingSection = "preference" | "system" | "memo-related" | "storage";

interface State {
  selectedSection: SettingSection;
}

const BASIC_SECTIONS: SettingSection[] = ["preference"];
const ADMIN_SECTIONS: SettingSection[] = ["system", "memo-related", "storage"];
...
const Setting = () => {
  ...
  const { fetchSetting } = useInstance();
  const [state, setState] = useState<State>({
    selectedSection: "preference",
  });
              ...
              {isHost ? (
                <>
                  ...
                  <div className="w-full flex flex-col justify-start items-start mt-1">
                    ...
                    <span className="px-3 mt-2 opacity-70 text-sm">Memore v0.1.7</span>
                  </div>
                </>
              ) : null}
            </div>
