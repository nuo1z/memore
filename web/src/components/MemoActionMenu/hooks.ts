...
import { findRemoteMemoNameByLocal, normalizeMemoreRemoteServerUrl } from "@/lib/memore-sync";
...
export const useMemoActionHandlers = ({ memo, onEdit, setDeleteDialogOpen }: UseMemoActionHandlersOptions) => {
  ...
  const handleCopyLink = useCallback(() => {
    const remoteUrl = normalizeMemoreRemoteServerUrl(syncPreferences.remoteServerUrl);
    if (remoteUrl) {
      const remoteMemoName = findRemoteMemoNameByLocal(remoteUrl, memo.name);
      if (remoteMemoName) {
        copy(`${remoteUrl}/${remoteMemoName}`);
    ...
    copy("");
    toast(t("message.no-remote-link"), { icon: "ℹ️" });
  }, [memo.name, t, syncPreferences.remoteServerUrl]);
