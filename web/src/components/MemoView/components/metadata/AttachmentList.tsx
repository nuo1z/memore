...
const DocumentItem = ({ attachment }: { attachment: Attachment }) => {
  ...
  const handleDownload = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    downloadFile(getAttachmentUrl(attachment), attachment.filename);
  };
  ...
  return (
    <div className="flex items-center gap-1 px-1 py-1 rounded text-xs text-muted-foreground hover:text-foreground hover:bg-accent/20 transition-colors whitespace-nowrap">
      ...
      <button
        type="button"
        className="shrink-0 w-5 h-5 flex items-center justify-center rounded hover:bg-accent/40 transition-colors"
...
const MediaItem = ({ attachment, onImageClick }: MediaItemProps) => {
  ...
  return (
    <div
      className="relative aspect-square rounded-lg overflow-hidden bg-muted/40 border border-border hover:border-accent/50 transition-all cursor-pointer group"
      onClick={handleClick}
    >
      <AttachmentCard attachment={attachment} className="rounded-none" />
      <button
        type="button"
        className="absolute bottom-1 right-1 w-6 h-6 rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm"
        onClick={handleDownload}
        title={`下载 ${attachment.filename}`}
      >
...
const DocsList = ({ attachments }: { attachments: Attachment[] }) => (
  <div className="flex flex-col gap-0.5">
    {attachments.map((attachment) => (
      <DocumentItem key={attachment.name} attachment={attachment} />
    ))}
  </div>
);
