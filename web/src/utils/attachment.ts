import { buildMemosApiUrl } from "@/lib/memos-api-base-url";
...
export const getAttachmentUrl = (attachment: Attachment) => {
  ...
  return buildMemosApiUrl(`/file/${attachment.name}/${attachment.filename}`);
};

export const getAttachmentThumbnailUrl = (attachment: Attachment) => {
  return buildMemosApiUrl(`/file/${attachment.name}/${attachment.filename}?thumbnail=true`);
};
