...
import { useMemoreStartupSync } from "@/hooks/useMemoreStartupSync";
import { useMemoreTriggeredSync } from "@/hooks/useMemoreTriggeredSync";
...
function AppInitializer({ children }: { children: React.ReactNode }) {
  ...
  // 启动阶段自动同步（受用户偏好开关控制）
  useMemoreStartupSync(authInitialized && instanceInitialized && !!currentUser);

  // 事件驱动同步：本地保存后触发、手动按钮触发
  useMemoreTriggeredSync(authInitialized && instanceInitialized && !!currentUser);
