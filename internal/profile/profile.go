// Package profile 提供服务器启动配置（Profile）的定义与校验。
// 包含数据目录路径推断（Windows: %ProgramData%/memore, Linux: /var/opt/memos 或当前目录）、
// DSN 生成（SQLite 默认文件名）等逻辑。
package profile
...
func (p *Profile) Validate() error {
	...
	if p.Data == "" {
		if runtime.GOOS == "windows" {
			p.Data = filepath.Join(os.Getenv("ProgramData"), "memore")
		} else {
	...
	if p.Driver == "sqlite" && p.DSN == "" {
		...
		dbFile := fmt.Sprintf("memore_%s.db", mode)
